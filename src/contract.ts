...
export default class Contract {
  ...
  constructSend(
    ...
    return async (...args: CodecArg[]): Promise<null> => {
      ...
      const maximumBlockWeight =
        this._provider.api.consts.system.blockWeights.maxBlock;

      await new Promise((resolve, reject) => {
        this._provider.api.tx.contracts
          .call(this._address, 0, maximumBlockWeight, encodedParams)
          .signAndSend(
            this._account.getAddress(),
            { signer: this._account.getSigner() },
