...
export default class ContractFactory {
  ...
  async deploy(params: CodecArg[], { weight, value }: { weight: number, value: number }): Promise<Contract> {
      ...
      const transactionCallback = (result: SubmittableResult) => {
        if (result.status.isInBlock) {
          ...
          if (successEvents.length > 0) {
            const instantiateEvents = result.filterRecords('contracts', 'Instantiated');
            const lastInstantiatedEvent = instantiateEvents[instantiateEvents.length - 1];
            const account = lastInstantiatedEvent.event.data[1] as AccountId;
            const contract = new Contract(this.provider, account.toHuman(), this.account, this.code)
            resolve(contract);
          }
